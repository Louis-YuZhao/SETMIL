...
from util import parse_patch_fname as pf

def parse_patch_fname_for_dsmil(fp: str) -> Tuple[int, int]:
...
class WSIFeatDataset(data_utils.Dataset):
    ...
    def __init__(self, ann_file=None, transforms=None, num_class=2, is_training=False,
                 feat_map_size=60, cache_mode=False, local_rank=0, local_size=1):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        try:
            for each_ob in patch_feat:
                ...
                #patch_loc.append(parse_patch_fname_for_dsmil(feat_name))
...
def build(image_set, args):
    ...
    dataset = WSIFeatDataset(transforms=make_wsi_transforms(image_set),
                         ann_file=ann_file,
                         num_class=num_class,
                         cache_mode=cache_mode, local_rank=get_local_rank(), local_size=get_local_size(),
