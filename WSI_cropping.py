...
def crop_slide(img, save_slide_path, mark, position=(0, 0), step=(0, 0), patch_size=224, scale=10, down_scale=1): # position given as (x, y) at nx scale
    ...
    # print(position[0] * down_scale, position[1] * down_scale, patch_size * down_scale)
...
def extract_coord_from_xml(xml_path: str) -> list:
    ...
    for annot in xml_doc.iterfind('Annotations/Annotation'):
        ...
        polygon = polygon.buffer(0.01) # added by lzy
        coord_group.append({
            'Name': annot_name,
            'Color': annot_color,
            'Coords': coords,
...
if __name__ == '__main__':
    ...
    out_base = join(args.output, str(args.scale))
